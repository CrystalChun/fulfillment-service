...
def main() -> int:
    parser = argparse.ArgumentParser(description="Disable access key")
    ...
    try:
        # First, we need to find the access key ID in our system
        # The access_key_id from the argument is the public identifier
        # We need to call the disable endpoint

        response = requests.post(
            f"{args.base_url}/api/fulfillment/v1/organizations/{args.organization_id}/access-keys/{args.access_key_id}:disable",
            headers=headers,
            json={},
            timeout=30,
        )
        ...
        if response.status_code not in (200, 201):
            result["error"] = f"Failed to disable access key: HTTP {response.status_code}"
            try:
