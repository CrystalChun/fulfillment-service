...
func (b *UsersServerBuilder) SetIdpClient(value idp.Client) *UsersServerBuilder {
	b.idpClient = value
	return b
}
...
func (s *UsersServer) List(ctx context.Context,
	...
	for i, privateItem := range privateItems {
		...
		if err != nil {
			s.logger.ErrorContext(ctx, "Failed to map user to public format", slog.Any("error", err))
			return nil, err
		}
...
func (s *UsersServer) Get(ctx context.Context,
	...
	publicUser := &publicv1.User{}
	err = s.outMapper.Copy(ctx, privateResponse.GetObject(), publicUser)
	if err != nil {
		s.logger.ErrorContext(ctx, "Failed to map user to public format", slog.Any("error", err))
		return nil, err
	}
...
func (s *UsersServer) Create(ctx context.Context,
	...
	privateUser := &privatev1.User{}
	err = s.inMapper.Copy(ctx, request.GetObject(), privateUser)
	if err != nil {
		s.logger.ErrorContext(ctx, "Failed to map user to private format", slog.Any("error", err))
		return nil, err
	}
