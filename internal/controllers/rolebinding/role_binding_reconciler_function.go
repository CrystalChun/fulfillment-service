...
func (t *task) handleUserListChange(ctx context.Context) error {
	...
	syncedUsers := t.binding.GetStatus().GetSyncedUsers()
	...
	for _, userID := range usersToRemove {
		...
		if clientID != "" {
			err = t.r.idpClient.RemoveClientRolesFromUser(ctx, organizationName, idpUserID, clientID, keycloakRoles)
		} else {
			err = t.r.idpClient.RemoveOrganizationRolesFromUser(ctx, organizationName, idpUserID, keycloakRoles)
		}
	...
	for _, userID := range usersToAdd {
		...
		if clientID != "" {
			err = t.r.idpClient.AssignClientRolesToUser(ctx, organizationName, idpUserID, clientID, keycloakRoles)
		} else {
			err = t.r.idpClient.AssignOrganizationRolesToUser(ctx, organizationName, idpUserID, keycloakRoles)
		}
	...
	if len(assignmentErrors) > 0 || len(removalErrors) > 0 {
		...
		t.binding.GetStatus().SetSyncedUsers(desiredUsers)
...
func (t *task) syncRoleAssignments(ctx context.Context) error {
	...
	if len(assignmentErrors) > 0 {
		...
		t.binding.GetStatus().SetSyncedUsers(t.binding.GetSpec().GetUsers())
	}
